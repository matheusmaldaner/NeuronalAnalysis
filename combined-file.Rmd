...
data_split <- initial_split((data_to_cluster_t), prop = 0.80)
...
validation_split <- initial_split(train_data, prop = 0.75)
train_final <- training(validation_split)   # Final training set (60% of original)
validation_data <- testing(validation_split) # Validation set (20% of original)

#build the model
random_forest_model <- rand_forest(
  trees = 500       # Default = 500
) %>%  
  set_engine("randomForest") %>% 
  set_mode("classification")

# create a recipe to define the preprocessing steps to prep data for model
recipe <- recipe(title_status ~ ., data = train_final) %>% 
...
# predict on train set, esnure that title is a factor and convert prediction to a factor
test_predictions <- predict(fit, test_data)
test_data$title_status <- as.factor(test_data$title_status)
test_predictions$.pred_class <- as.factor(test_predictions$.pred_class)


# calculate metrics
test_metrics <- test_predictions %>%
                      bind_cols(test_data) %>%
                      metrics(truth = title_status, estimate = .pred_class) %>%
                      filter(.metric %in% c("accuracy", "precision", "recall", "f_meas"))

print(test_metrics)

# Predict on the validation set
validation_predictions <- predict(fit, validation_data)

# Ensure that title_status in validation_data is a factor
validation_data$title_status <- as.factor(validation_data$title_status)

# Convert prediction to a factor (if it's not already)
validation_predictions$.pred_class <- as.factor(validation_predictions$.pred_class)
...
test_set_predictions <- predict(fit, test_data)
test_data$title_status <- as.factor(test_data$title_status)
test_set_predictions$.pred_class <- as.factor(test_set_predictions$.pred_class)
test_set_metrics <- test_set_predictions %>%
                    bind_cols(test_data) %>%
                    metrics(truth = title_status, estimate = .pred_class) %>%
                    filter(.metric %in% c("accuracy", "precision", "recall", "f_meas"))

# Print out the test set metrics
print(test_set_metrics)

# Extract gene Signatures 
random_forest_model <- extract_fit_parsnip(fit)
rf_model <- random_forest_model$fit

feature_importance <- importance(rf_model)
# Viewing the top important features
