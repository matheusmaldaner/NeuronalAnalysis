...
rf_auc <- 
  ...
  roc_curve(title_status, .pred_PM) %>% ##shows predictions for pm can change to other
...
roc_results <- roc_auc(test_predictions, truth = title_status, .pred_PM)
roc_results
...
top_20_indices = c(2424, 228, 4884, 103, 4299, 2813, 1050, 122, 1969, 139, 4025, 252, 141, 2613, 1055, 1899, 1654, 1895, 1316, 3161)  

top_gene_symbols = most_var_5000[top_20_indices, 2]
top_gene_symbols #top 20 important

import_fut <- rf_fit %>% extract_fit_parsnip()
importance_scores <- vip::vi(import_fut, num_features=nrow(train_data))
importance_scores # shows all variables and importance

#runs with mult

gene_sets <- list(df_10 = gene_expression[most_var_10[,1], ], df_100 = gene_expression[most_var_100[,1], ], df_1000 = gene_expression[most_var_1000[,1], ], df_10000 = gene_expression[most_var_10000[,1], ])
results <- list()
results_auc <- list()
...
test_predictions$title_status <- as.factor(test_predictions$title_status)
roc_results <- roc_auc(test_predictions, truth = title_status, .pred_PM) #can change to other depends on what score to identify
...
significant_genes <- importance_scores[importance_scores$Importance > 0.1, c("gene_symbol", "Importance", "Variable")]
...
pheatmap(
  ...
  fontsize_row = 6,
  fontsize_col = 6,
  main = "Heatmap of Significant Genes Above 0.1 Importance over 5,000 Genes",
