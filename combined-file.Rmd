...
data_split <- initial_split(data.frame(data_to_cluster, subset_labels), prop = 0.80)
...
recipe <- recipe(subset_labels ~ ., data = train_final) %>% 
...
validation_results <- fit %>%
                      predict(validation_data) %>%
                      bind_cols(validation_data) %>%
                      metrics(truth = subset_labels, estimate = .pred_class)
